...
document.getElementById('play').addEventListener('click', async () => {
  ...
  if (!utils.canConstructWord(rack.getAvailableTiles(), remaining)) {
    ...
    try {
      ...
      //       Save the player's name, word, and score to the server. `POST /wordScore` endpoint to do this. 
      //
      //       `await saveWordScore(name, word, score);`
      //        
      //       Update the word score table in the UI.
      //       For example:
      //
...
document.getElementById('end').addEventListener('click', async () => {
  // Add a button whose id is `end` 
  // Save the game scores to the server.
  // `POST /gameScore` endpoint to do this.
  //
